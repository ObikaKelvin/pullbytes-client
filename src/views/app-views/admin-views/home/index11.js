...
import { Row, Col, Button, Card, Table, Tag, Select, Badge, Skeleton } from 'antd';
...
const Home = () => {
	...
	const display_percent_increase = () => {
		const { percent_increase } = revenues;
		let textColor = '';
		let icon = '';
		...
		return(<p className={`text-${textColor}`}>
			<span >
				{ icon }
				...
				<Row gutter={16}>
					<Col xs={24} sm={24} md={24} lg={8}>
						<Flex className="h-100" flexDirection="column" justifyContent="between">
							...
							<div className="mb-4">
								<h1 className="font-weight-bold">${revenues.current_month_revenue}</h1>
